...
/**
  ...
  *
  *   val (fPlus, fMinus, cacheKeys) = matrix.dotprod(wInput, wOutput, 1) // compute dot products for gradient updates
  *   matrix.adjust(fPlus, gMinus, cacheKeys) // adjust matrix by gradient updates
  *
...
trait BigWord2VecMatrix extends BigMatrix[Float] {
  ...
  /**
    ...
    * @param ec The implicit execution context in which to execute the request
    * @return A future containing the gradient updates and the cache keys for the adjust operation
    */
